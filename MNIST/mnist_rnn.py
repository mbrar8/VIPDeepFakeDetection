...
rnncell = tfkl.SimpleRNN(128)(input)
...
tb = tf.keras.callbacks.TensorBoard(log_dir='rnn_logs')


start = time.time()
# Run the model. 5 epochs and 128 batch size (128 samples used for each run through before updating parameters)
trainResults = model.fit(trainX, trainY, epochs=5, validation_data=(valX, valY), callbacks=[tb])
end = time.time()
